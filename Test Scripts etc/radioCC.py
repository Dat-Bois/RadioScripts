script_version = "Script Version 2.0" 
script_date    = "Script Date : 9-30-2018"  
...
def transmit(channel):
	...
	#Setting target IP for channel transmission
	UDP_IP = "192.168.1.46"
	UDP_IPL = "192.168.1.100"
	...
	if channel == "stop":
		...
		channel = int(channel)
		url = ChannelsCC.channelList[channel].getUrl()
		MESSAGE = str(url)
	
	print ("-----")
	print ("Sending to target IP: ", UDP_IP)
	print ("Sending to port: ", UDP_PORT)

	sock = socket.socket(socket.AF_INET, # Internet
						socket.SOCK_DGRAM) # UDP
	sock.sendto(MESSAGE, (UDP_IP, UDP_PORT))
	sock.sendto(MESSAGE, (UDP_IPL, UDP_PORT))
